...
public class AnimatorInflaterCompat {
    ...
    private static void setupPathMotion(Path path, ObjectAnimator oa, float precision, String propertyXName, String propertyYName) {
        ...
        PathMeasure measureForTotalLength = new PathMeasure(x, false);
        ...
        PropertyValuesHolder x2 = null;
        measureForTotalLength = null;
        if (str != null) {
            x2 = PropertyValuesHolder.ofFloat(str, mX);
        }
        if (str2 != null) {
        ...
        if (x2 == null) {
            ...
            objectAnimator.setValues(new PropertyValuesHolder[]{x2});
        } else {
            objectAnimator.setValues(new PropertyValuesHolder[]{x2, measureForTotalLength});
        }
    }
                    ...
                    if (keyframe2.getFraction() < f) {
                        if (next == 0) {
                            ...
                            while (true) {
                                ...
                                if (type >= count - 1) {
                                    break;
                                } else if (keyframeArray[type].getFraction() >= 0.0f) {
                                    break;
                                } else {
                                    value = type;
                                    j = type + 1;
