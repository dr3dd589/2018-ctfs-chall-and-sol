    ...
    public boolean isValidConnection(ConstraintAnchor anchor) {
        ...
        if (target != this.mType) {
            ...
            switch (this.mType) {
                ...
                case LEFT:
                case RIGHT:
                    isCompatible = target == Type.LEFT || target == Type.RIGHT;
                ...
                case TOP:
                case BOTTOM:
                    isCompatible = target == Type.TOP || target == Type.BOTTOM;
                    if (anchor.getOwner() instanceof Guideline) {
    ...
    public boolean isSnapCompatibleWith(ConstraintAnchor anchor) {
        ...
        switch (this.mType) {
            ...
            case NONE:
                return false;
            case LEFT:
                i = AnonymousClass1.$SwitchMap$android$support$constraint$solver$widgets$ConstraintAnchor$Type[anchor.getType().ordinal()];
                if (i == 3 || i == 7) {
                    return true;
                }
                return false;
